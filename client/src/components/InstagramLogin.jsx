...
const styles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ig-input-wrap { position: relative; width: 100%; }
  ...
  .ig-input:focus { border-color: #a8a8a8; background: #fff; }
  ...
  .ig-terms a { color: #8e8e8e; font-weight: 700; text-decoration: none; }
...
export default function InstagramLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setAlert({ type: "success", msg: `✅ Welcome back, ${data.username}! Login saved to database.` });
        setUsername("");
        setPassword("");
      } else {
      ...
      setAlert({ type: "error", msg: "Cannot connect to server. Make sure backend is running." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setAlert({ type: "success", msg: `✅ Account created! You can now log in as ${data.username}` });
  ...
  const isRegisterValid = username.trim().length > 0 && email.trim().length > 0 && password.length > 0;
      ...
      <div className="ig-body">
        <div className="ig-container">
          <div className="ig-card">
            ...
            {alert && (
              <div className={`ig-alert ig-alert-${alert.type}`}>{alert.msg}</div>
            )}
            ...
            <form className="ig-form" onSubmit={handleLogin}>
              ...
              <div className="ig-input-wrap">
                ...
                />
                {password && (
                  <button type="button" className="ig-show-btn" onClick={() => setShowPass(!showPass)}>
                    {showPass ? "Hide" : "Show"}
                  </button>
                )}
              </div>

              <button type="submit" className="ig-login-btn" disabled={!isLoginValid || loading}>
          ...
          <div className="ig-signup-card">
            ...
            <a onClick={() => switchPage("register")}>Sign up</a>
          </div>
  ...
  return (
    <div className="ig-body">
      <div className="ig-container">
        <div className="ig-card">
          <div className="ig-logo">Instagram</div>
          <p className="ig-tagline">Sign up to see photos and videos from your friends.</p>

          <button className="ig-fb-login" type="button" style={{ marginBottom: 14 }}>
            <FacebookIcon />
          ...
          <form className="ig-form" onSubmit={handleRegister}>
            <div className="ig-input-wrap">
              <input
                className="ig-input"
                type="text"
            ...
            <button type="submit" className="ig-login-btn" disabled={!isRegisterValid || loading}>
              {loading && <span className="ig-spinner" />}
        ...
        <div className="ig-signup-card">
          Have an account?{" "}
          <a onClick={() => switchPage("login")}>Log in</a>
        </div>
